...
@RestController
@RequestMapping("/pro/log")
public class JobLogApiController {
    private static Logger logger = LoggerFactory.getLogger(JobLogApiController.class);
    ...
    public ReturnT pageList(@RequestParam(required = false, defaultValue = "0") int start,
        ...
        // page query
        List<XxlJobLog> list = xxlJobLogDao.pageList(start, length, jobGroup, jobId, triggerTimeStart, triggerTimeEnd, logStatus, Integer.parseInt(UserInterceptor.projectThread.get()));
        int list_count = xxlJobLogDao.pageListCount(start, length, jobGroup, jobId, triggerTimeStart, triggerTimeEnd, logStatus, Integer.parseInt(UserInterceptor.projectThread.get()));
        if (logStatus == 0 && jobId > 0) {   //统计成功率
    ...
    public ReturnT logDetailPage(int id) {
        ...
        if (jobLog.getProjectId() != Integer.parseInt(UserInterceptor.projectThread.get())) {
            return ReturnT.FAIL;
        }
    ...
    public ReturnT<String> logKill(int id) {
        ...
        if (Integer.parseInt(UserInterceptor.projectThread.get()) != jobInfo.getProjectId()) {
            return new ReturnT<String>(500, "禁止操作");
        }
    ...
    public ReturnT<String> clearLog(int jobGroup, int jobId, int type) {
        ...
        do {
            logIds = xxlJobLogDao.findClearLogIds(jobGroup, jobId, clearBeforeTime, clearBeforeNum, 1000, Integer.parseInt(UserInterceptor.projectThread.get()));
